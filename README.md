...
- **Medición de precisión**: Firmware para Arduino y Raspberry Pi Pico (MicroPython) que calcula la velocidad detectando el paso del proyectil por barreras de sensores ópticos (infrarrojos).
...
├── imported_cronografo/      # Firmware para dispositivos de hardware (Arduino y Raspberry Pi Pico)
│   ├── arduino/              # Código C++ (.ino) para Arduino
│   └── (archivos .py)        # Scripts de MicroPython para Raspberry Pi Pico (con soporte MQTT, Web y OLED)
...
- **Arduino**: Código fuente en `imported_cronografo/arduino/cronoABC ultima version/cronoABC/cronoABC.ino`.
- **Raspberry Pi Pico (MicroPython)**:
  - `main.py`: Script principal de control y lectura de sensores.
  - `ssd1306.py`: Controlador para pantallas OLED.
  - `umqtt.py` / `uhttp.py`: Protocolos de comunicación para subir datos a servidores locales o MQTT.
